...
class PlotTable:
    ...
    def PlotCurve(self):
        ...
        if self.plotType == "Bler":
            ...
            for n in range(len(TempPlot[0])):
                plt.plot(TempPlot[0][n], TempPlot[1][n], label = str(self.label[0][n][1]) + " " + str(self.label[1][n][3]), linestyle = self.lineSytle
    ...
    def CreateCurveDataBler(self, TableType, SnrMin, SnrMax, Resolution):
        ...
        if TableType == "CqiB":     
            for n in range(len(self.SnrVector)):
                DataY.append(CalculationBlerEfficiency((Tables().getTableCqiB())[0][0], (Tables().getTableCqiB())
                                [0][1], (Tables().getTableCqiB())[0][2]).GetBler(self.SnrVector[n]))
            for i in range(1, len(Tables().getTableCqiB())):
                    for n in range(len(self.SnrVector)):
                        TempDataY.append(CalculationBlerEfficiency((Tables().getTableCqiB())[i][0], (Tables().getTableCqiB())
                                        [i][1], (Tables().getTableCqiB())[i][2]).GetBler(self.SnrVector[n]))
                    DataY = np.row_stack((DataY, TempDataY))
                    TempDataY = []    
        elif TableType == "McsA":
            for n in range(len(self.SnrVector)):
                        DataY.append(CalculationBlerEfficiency((Tables().getTableMcsA())[0][0], (Tables().getTableMcsA())
                                        [0][1], (Tables().getTableMcsA())[0][2]).GetBler(self.SnrVector[n]))
            for i in range(1, len(Tables().getTableMcsA())):
                    for n in range(len(self.SnrVector)):
                        TempDataY.append(CalculationBlerEfficiency((Tables().getTableMcsA())[i][0], (Tables().getTableMcsA())
                                        [i][1], (Tables().getTableMcsA())[i][2]).GetBler(self.SnrVector[n]))
                    DataY = np.row_stack((DataY, TempDataY))
                    TempDataY = []    
        elif TableType == "McsB":
            for n in range(len(self.SnrVector)):
                        DataY.append(CalculationBlerEfficiency((Tables().getTableMcsB())[0][0], (Tables().getTableMcsB())
                                        [0][1], (Tables().getTableMcsB())[0][2]).GetBler(self.SnrVector[n]))
            for i in range(1, len(Tables().getTableMcsB())):
                    for n in range(len(self.SnrVector)):
                        TempDataY.append(CalculationBlerEfficiency((Tables().getTableMcsB())[i][0], (Tables().getTableMcsB())
                                        [i][1], (Tables().getTableMcsB())[i][2]).GetBler(self.SnrVector[n]))
                    DataY = np.row_stack((DataY, TempDataY))
                    TempDataY = []    
    ...
    def CreateCurveDataEfficiency(self, TableType, SnrMin, SnrMax, Resolution):
        ...
        if TableType == "CqiB":
            for n in range(len(self.SnrVector)):
                        DataY.append(CalculationBlerEfficiency((Tables().getTableCqiB())[0][0], (Tables().getTableCqiB())
                                        [0][1], (Tables().getTableCqiB())[0][2]).GetEfficiency(self.SnrVector[n]))
            for i in range(1, len(Tables().getTableCqiB())):
                    for n in range(len(self.SnrVector)):
                        TempDataY.append(CalculationBlerEfficiency((Tables().getTableCqiB())[i][0], (Tables().getTableCqiB())
                                        [i][1], (Tables().getTableCqiB())[i][2]).GetEfficiency(self.SnrVector[n]))
                    DataY = np.row_stack((DataY, TempDataY))
                    TempDataY = []    
        elif TableType == "McsA":
            for n in range(len(self.SnrVector)):
                        DataY.append(CalculationBlerEfficiency((Tables().getTableMcsA())[0][0], (Tables().getTableMcsA())
                                        [0][1], (Tables().getTableMcsA())[0][2]).GetEfficiency(self.SnrVector[n]))
            for i in range(1, len(Tables().getTableMcsA())):
                    for n in range(len(self.SnrVector)):
                        TempDataY.append(CalculationBlerEfficiency((Tables().getTableMcsA())[i][0], (Tables().getTableMcsA())
                                        [i][1], (Tables().getTableMcsA())[i][2]).GetEfficiency(self.SnrVector[n]))
                    DataY = np.row_stack((DataY, TempDataY))
                    TempDataY = []    
        elif TableType == "McsB":
            for n in range(len(self.SnrVector)):
                        DataY.append(CalculationBlerEfficiency((Tables().getTableMcsB())[0][0], (Tables().getTableMcsB())
                                        [0][1], (Tables().getTableMcsB())[0][2]).GetEfficiency(self.SnrVector[n]))
            for i in range(1, len(Tables().getTableMcsB())):
                    for n in range(len(self.SnrVector)):
                        TempDataY.append(CalculationBlerEfficiency((Tables().getTableMcsB())[i][0], (Tables().getTableMcsB())
                                        [i][1], (Tables().getTableMcsB())[i][2]).GetEfficiency(self.SnrVector[n]))
                    DataY = np.row_stack((DataY, TempDataY))
                    TempDataY = []    
        return [np.matlib.repmat(self.SnrVector, len(DataY), 1), DataY]
